...
def create_agent(name: str = Form(...), status: str = Form("offline"), user: str = Form("operator")):
    clean_name = name.strip()
    if not clean_name:
        return RedirectResponse(f"/agents?{urlencode({'user': user})}", status_code=303)

    with Session(engine) as session:
        session.add(Agent(name=clean_name, status=("online" if status == "online" else "offline")))
        session.add(
            AuditLog(
                user=user,
                action="CREATE_AGENT",
                details=f"Добавлен агент: {clean_name}",
            )
        )
        session.commit()
    return RedirectResponse(f"/agents?{urlencode({'user': user})}", status_code=303)
...
def delete_agent(agent_id: int, user: str = Form("operator")):
    with Session(engine) as session:
        ...
        session.exec(delete(Telemetry).where(Telemetry.agent_id == agent_id))
        session.exec(delete(AuditLog).where(AuditLog.agent_id == agent_id))
        session.delete(agent)
        session.add(
            AuditLog(
                user=user,
                action="DELETE_AGENT",
                details=f"Удалён агент: {agent_name}",
            )
        )
        session.commit()
    return RedirectResponse(f"/agents?{urlencode({'user': user})}", status_code=303)
