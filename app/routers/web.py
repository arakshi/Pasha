...
def agents_page(request: Request, search: str = "", status: str = "all", sort: str = "desc", user: str | None = None):
    ...
    return templates.TemplateResponse(
        request,
        "agents.html",
        {"user": cu, "title": PAGE_TITLES["agents"], "agents": agents, "latest": latest, "profiles": profiles, "search": search, "status": status, "sort": sort},
    )
...
def profiles_page(request: Request, search: str = "", sort: str = "name", user: str | None = None):
    ...
    return templates.TemplateResponse(
        request,
        "profiles.html",
        {"user": cu, "title": PAGE_TITLES["profiles"], "profiles": profiles, "search": search, "sort": sort},
    )
...
def audit_page(request: Request, user_filter: str = "", action_filter: str = "", user: str | None = None):
    ...
    return templates.TemplateResponse(
        request,
        "audit.html",
        {"user": cu, "title": PAGE_TITLES["audit"], "logs": logs, "user_filter": user_filter, "action_filter": action_filter},
    )
...
def analytics_page(request: Request, user: str | None = None):
    ...
    return templates.TemplateResponse(
        request,
        "analytics.html",
        {"user": cu, "title": PAGE_TITLES["analytics"], "summary": summary, "events": events, "explanations": _analytics_explanations(summary)},
    )
...
def tests_page(request: Request, user: str | None = None):
    ...
    for row in rows:
        ...
        expanded.append({"row": row, "checks": checks})

    success_rate = [{"day": d, "rate": round(sum(vals) / len(vals) * 100, 1)} for d, vals in sorted(per_day.items())]
    avg_duration = round(sum(r.duration_ms for r in rows) / len(rows), 1) if rows else 0
